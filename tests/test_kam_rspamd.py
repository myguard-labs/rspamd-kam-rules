...
class ConversionTests(unittest.TestCase):
    ...
    def test_body_length_dedup_uses_same_container_alternative(self):
        # multipart/alternative de-dup must be scoped to the alternative sibling
        # relationship, not "any HTML part in the message".
        converted, _, _ = kam_rspamd.convert(FIXTURE.read_bytes(), "test", 1, 1)
        text = converted.decode()
        self.assertIn("get_alt_part()", text)
        self.assertIn("mimepart_id", text)
        self.assertNotIn("local has_html", text)
...
class HelperFunctionTests(unittest.TestCase):
    ...
    def test_header_exists_rule(self):
        ...
        rules, omitted, _, _, _, _ = kam_rspamd.parse_rules(
