...
stages the new map at `cache_path` (`/var/lib/rspamd/kam_rules.map` — the one map dir
the dropped-privilege rspamd user can write). No host cron, no `curl`, no SHA-compare
...
The converter builds against **your** production symbol set, so the output adapts to
your stack:
...
- `config/local-rules.cf` — site-local rules in SpamAssassin syntax, compiled into the
  same output (`--local-rules <file>` to override).

Regenerate the symbol dump whenever you change stacks, then rebuild. For a pinned
local source, add `--expected-sha256 <hash>` — conversion fails on a byte mismatch.
