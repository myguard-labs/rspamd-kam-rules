...
# Full write-up / background article (carried in both artifact headers).
PROJECT_ARTICLE = "https://deb.myguard.nl/articles/kam-cf-rspamd-lua-converter/"
# Profile overview of our other Rspamd modules (olefy, yarad, gyzor, mailstrix, …).
PROJECT_OVERVIEW = "https://github.com/myguard-labs"

# ---------------------------------------------------------------------------
# LUA RUNTIME MODIFIED DATE — BUMP THIS EVERY TIME YOU EDIT THE LUA RUNTIME.
# ---------------------------------------------------------------------------
# The generated kam.lua is version-less by design (the ruleset version lives in
# the map header), so a daily map regen does NOT touch the lua. This date tracks
# the RUNTIME code instead: whenever you change any LUA_RUNTIME / generate_lua()
# output, bump LUA_MODIFIED to today and re-run `python3 kam_rspamd.py --emit-lua`
...
def parse_rules(
    ...
    # Names redefined within the combined source (KAM.cf + local supplements are
    # parsed as one stream). A local-rules / SA-lift rule with the same name as a
    # KAM.cf rule silently wins (last write). That is intentional (overrides are
    # how we patch KAM.cf), but it must be VISIBLE so an accidental shadow is
    # caught in review, not at runtime. Records name -> (old kind, new kind).
    overridden: dict[str, tuple[str, str]] = {}

    def define(name: str, rule: Rule) -> None:
        prior = rules.get(name)
        if prior is not None:
            overridden[name] = (prior.kind, rule.kind)
        rules[name] = rule
    ...
    for number, line in lines:
        ...
        if directive == "describe" and len(parts) == 3:
            ...
            if not VALID_NAME.match(parts[1]):
                ...
                define(parts[1], Rule(parts[1], "meta", parts[2]))
            ...
            if directive == "header":
                ...
                if exists:
                    ...
                    define(
                        name,
                        Rule(name=name, kind=directive, expression="^", header=exists.group(1)),
                    )
                    continue
    ...
    for name in dropped:
        rules.pop(name, None)
    # A rule (from KAM.cf or a supplement) whose name equals a builtin eval
    # symbol shadows the inline eval_atom computation: rules[name] correctly
    # wins in eval_atom, but silently. Surface it so an accidental clash with
    # __KAM_BODY_LENGTH_* / HTML_MESSAGE / __TAG_EXISTS_HEAD is reviewable.
    eval_shadowed = sorted(name for name in rules if name in PLUGIN_EVAL_SYMBOLS)
    return rules, omitted, examples, dropped, dict(sorted(overridden.items())), eval_shadowed
...
-- Approximation of SA check_body_length: SA renders one body alternative per
-- multipart/alternative container. rspamd's task:get_text_parts() returns both
-- text/plain and text/html siblings, so counting both makes the body look too
-- long and causes LT_* atoms to under-fire. text_part:get_alt_part() is
-- one-directional (only the html part resolves its plain sibling; the plain
-- part's own get_alt_part() is nil) -- so a plain part is skipped only if some
-- html part in the message points back at it via get_alt_part(). Lua userdata
-- has no value equality (`==` on two get_alt_part()/iteration results for the
-- same underlying part is false), so parts are compared via their stable
-- mimepart id (text_part:get_mimepart():get_id()) instead of `==`. Unrelated
-- plain text parts (no html part claims them as its alternative) still count.
local function mimepart_id(part)
  local mp = part.get_mimepart and part:get_mimepart()
  return mp and mp.get_id and mp:get_id()
end
local function sa_body_length(task)
  local parts = task:get_text_parts() or {}
  local total = 0
  for _, part in ipairs(parts) do
    local is_alt_of_html = false
    if not part:is_html() then
      local part_id = mimepart_id(part)
      for _, other in ipairs(parts) do
        if other:is_html() and other.get_alt_part then
          local alt = other:get_alt_part()
          if alt and part_id ~= nil and mimepart_id(alt) == part_id then
            is_alt_of_html = true
            break
          end
        end
...
def generate_map(
    ...
    header = {
        ...
        "_article": PROJECT_ARTICLE,
        "_overview": PROJECT_OVERVIEW,
        "_howto": PROJECT_HOWTO,
        "_source_url": source_url,
        "_source_sha256": source_sha256,
        # KAM sa-update channel serial (reversed-version scheme, from DNS TXT
        # 0.0.4.kam.sa-channels.mcgrail.com). Identifies the exact upstream
        # ruleset revision these rules were built from.
        "_serial": serial,
        "_generated": generated_date,
        "_generated_utc": generated_utc,
...
def generate_lua(map_path: str = DEFAULT_MAP_PATH) -> bytes:
    ...
    lines = [
        ...
        f"-- Modified: {LUA_MODIFIED}  (bump on every runtime edit; ruleset version is in the map)",
        f"-- Home:     {PROJECT_HOMEPAGE}",
        f"-- Article:  {PROJECT_ARTICLE}",
...
def convert(
    ...
    rules, omitted, examples, dropped, overridden, eval_shadowed = parse_rules(
        combined,
        external,
    ...
    generated_date = now.strftime("%Y-%m-%d")
    generated_utc = now.strftime("%Y-%m-%dT%H:%M:%SZ")
    # Best-effort: stamp the map with the KAM channel serial recorded in
    # dist/kam.serial. Absent/unreadable -> empty (map still valid). CI writes a
    # fresh serial in its own step; a byte-identical re-touch only bumps serial.
    serial = ""
    serial_file = Path(__file__).resolve().parent / "dist" / "kam.serial"
    try:
        serial = re.sub(r"\D", "", serial_file.read_text())
    except OSError:
        pass
    lua = generate_lua()
    mapdata = generate_map(
        rules, dependencies, source_url, source_sha256,
        generated_date, generated_utc, serial,
    )
    ...
    report = {
        ...
        # name -> [old_kind, new_kind] for rules redefined in the combined source
        # (a local/SA-lift rule shadowing a same-named KAM.cf rule, last wins).
        "overridden_rules": {name: list(kinds) for name, kinds in overridden.items()},
        "overridden_rule_count": len(overridden),
        # rules whose name collides with a builtin eval symbol (rule wins).
        "eval_shadowed_symbols": eval_shadowed,
